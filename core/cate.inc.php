...
function getCateById($id){
	$sql="select id,cName from boke_class where id={$id}";
	return fetchOne($sql);
}
...
function editCate($where){
	$arr=$_POST;
	if(update("boke_class", $arr,$where)){
...
function delCate($id){
	...
	if(!$res){
		...
		alertMes("不能删除分类，请先删除该分类下的文章", "listLog.php");
	}
}
 ...
 */
function getAllCate(){
	$sql="select id,cName from boke_class";
	$rows=fetchAll($sql);
	return $rows;
}
